...
RSpec.configure do |config|
  ...
  def sandbox
    File.join(File.dirname(__FILE__), '..', 'tmp', 'sandbox')
  end

  def run(command)
    mkdir_p sandbox
    cd(sandbox) do
      # puts "in run: Dir.pwd: #{Dir.pwd.inspect}"
      system("#{command}")
    end
  end
  ...
  def setup_existing_project_in_sandbox
    mkdir_p File.join(sandbox, 'lib', 'sandbox')
    mkdir_p File.join(sandbox, 'spec')    
  end
